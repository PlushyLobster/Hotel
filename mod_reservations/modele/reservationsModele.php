...
class ReservationsModele extends modele
{
    ...
    public function insertUnereservation(){
        ...
        $sql = "INSERT INTO reservations (ResNo, ResDate, LastName, FirstName, Address, City, State, Postal, Phone, Payment, Amount, Hotel, Room, DateIn, DateOut) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

        $resultat = $this->executeRequete($sql, array($idResNo, $this->parametre['ResDate'], $this->parametre['LastName'], $this->parametre['FirstName'], $this->parametre['Address'], $this->parametre['City'], $this->parametre['State'], $this->parametre['Postal'], $this->parametre['Phone'], $this->parametre['Payment'], $this->parametre['Amount'], $this->parametre['Hotel'], $this->parametre['Room'], $this->parametre['DateIn'], $this->parametre['DateOut']));
        $uneReservation = new ReservationsTable($resultat->fetch(PDO::FETCH_ASSOC));
        return $uneReservation;
    }
}
